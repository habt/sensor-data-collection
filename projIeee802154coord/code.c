...
uint16_t packet_ack = 0; //16 bit integer
...
static void rx_data(int8_t status, uint8_t *data, uint8_t len, uint16_t addr)
{
	if (len <= TEST_PACKET_SIZE) {
		memcpy(test_packet, data, len); //copies recieved data to test packet
...
TASK(CHAT_SEND)
{
	...
	packet[6] = tx_counter >> 8; // left most eight bits of tx_counter selected by shifting to the right and given to pack 6
	packet[7] = tx_counter & 0x00FF; //right most eight bits selected
...
TASK(CHAT_RECEIVE)
{
	...
	packet_ack = test_packet[7];//right most 8 bits of packet ack from test packet 7
	packet_ack += ((uint16_t) test_packet[6]) << 8; // cast test pack 6 to 16 bits then shift to the left then add pack ack , right most packet ack is from test pack 6
	/* Check the packet ack counter (should be in sequence) */
	if (packet_ack != rx_counter)
		rx_seq_errors++;
	rx_counter = packet_ack; //number recieved as number of this packet
	rx_counter++;

	//Receive accelerometer data
	x_level = test_packet[13];
	x_level += ((int)test_packet[12]) << 8;
	x_level += ((int)test_packet[11]) << 16;
	x_level += ((int)test_packet[11]) << 24;

	y_level = test_packet[17];
	y_level += ((int)test_packet[16]) << 8;
	y_level += ((int)test_packet[15]) << 16;
	y_level += ((int)test_packet[14]) << 24;

	z_level = test_packet[21];
	z_level += ((int)test_packet[20]) << 8;
	z_level += ((int)test_packet[19]) << 16;
	z_level += ((int)test_packet[18]) << 24;
	...
	/* Print stats on LCD */
	//sprintf(str, "\nCHAT:C ETX=%u\nRX=%u ESEQ=%u",tx_counter - tx_success, rx_counter, rx_seq_errors);
	sprintf(str1, "%f %f %f \r\n",x_level/100.0,y_level/100.0,z_level/100.0);

	/* console part added begin */
	static char j = 0;
	console_write(MY_FIRST_CONSOLE, str1, strlen(str1));
		if ((j++)%2)
			EE_leds_on();
		else
			EE_leds_off();
		/* console part added end */

	sprintf(str2, "X=%d Y=%d\nZ=%d \r\n",x_level,y_level,z_level);
	lcd_puts(str2);

	/* Activate the SEND task */
	ActivateTask(CHAT_SEND);  // Diffrence from device - coordinator sends reply whenever it recieves data from device
							// no periodic alarm to send data back to device => only returns after recieving
}
...
int main(void)
{
	...
	timer_set();
	EE_leds_init();     EE_demoboard_leds_init();
