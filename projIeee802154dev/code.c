...
TASK(CHAT_SEND)
{
	...
	int * x_acc_int_ptr = (int *) (& x_level) ;
	int * y_acc_int_ptr = (int *) (& y_level) ;
	int * z_acc_int_ptr = (int *) (& z_level) ;

	//Here write the accelerometer sensing and sending part with 12 packets
	//use packets 10 - 22;
	...
	//Here obtain values from accelerometer and put them in packets
...
TASK(CHAT_RECEIVE)
{
	...
	packet_ack = test_packet[7]; // right most 8 bits of packet ack given value of test packet 7
	packet_ack += ((uint16_t) test_packet[6]) << 8;//shift test pack6 8 to left and add to packet ack => test 6 is 8 left most bits of packet ack
...
int main(void)
{
	...
	SetRelAlarm(CHAT_SEND_ALARM, CHAT_SEND_OFFSET,
					    CHAT_SEND_PERIOD);          //Diffrence from coord - device here has a periodically send triggering alarm
